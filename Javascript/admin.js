...
window.addEventListener("load", () => {
    ...
    const ultimoID = Number((productos[productos.length -1]).valor)
    document.getElementsByName('id')[0].value=(ultimoID+1)
    }
    for (let i = 0; i < productos.length; i++) {
    ...
    for (let i = 0; i < productos.length; i++) {
        mostraProductos.innerHTML += ` <tr><td>${productos[i].valor}</td>
        <td>${productos[i].nombre}</td>
        <td>${productos[i].stock}</td></tr>
        `
    }
})
//`<div class="contenedorProductos"><div class="informacion"><p>${productos[i].nombre}</p><p class="precio"><span>Precio: ${productos[i].valor}$</span></p> Existencia: ${productos[i].existencia}<p></p></div></div>`
